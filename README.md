...
logging, tests, a read-only connection to Bybit's public REST API that
discovers the current market universe (linear, USDT-settled, perpetual,
currently tradable instruments), a continuously-running MARKET collector
...
`market_candles`' monthly partitions are the one exception: the migration
that creates the table only provisions a handful of partitions around the
current month so the app can start immediately. Rolling the partition
window forward and dropping expired months is an ongoing operational
concern owned by `app/services/partition_manager.py`, run from the
application's background lifecycle rather than by further migrations.
